...
vignette: >
  %\VignetteIndexEntry{hecr 2D Recipes}
...
library(hecr)
path = "C:\\Example_Projects_6_6\\2D Sediment Transport\\Chippewa_2D\\Chippewa_2D.p01.hdf"
...
hdf_contents <- rhdf5::h5ls(paste0("C:\\Example_Projects_6_6\\2D Sediment Transport\\Chippewa_2D\\Chippewa_2D.p01.hdf"))

cel_centre_info <- hdf_contents %>%
  dplyr::filter(.data$name == "Cells Center Coordinate")

comp_cell_coords <- rhdf5::h5read(paste0("C:\\Example_Projects_6_6\\2D Sediment Transport\\Chippewa_2D\\Chippewa_2D.p01.hdf"),
                                  name = paste0(cel_centre_info$group,"/",
                                                cel_centre_info$name)) %>%
  tibble::as_tibble() %>%
  t() %>%
  tibble::as_tibble() %>%
  dplyr::rename("x" = "V1",
  ...
  sf::st_as_sf(coords = c("x","y"), crs = "ESRI:102039") %>%
...
terra::crs(r) <- "ESRI:102039"
extent(r) <- terra::extent(-180, 180, -90, 90)

r_check <- r[[1]]$`2D_Iterations`

names(h$object)

library(hdf5r)

sediment.grp <- h$object$get_obj_name()
...
# get full coord range from hdf


data <- hec_two(hc = h,xy = )
